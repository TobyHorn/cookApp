...
$.getJSON('https://api.myjson.com/bins/f4ayd', function (data) {
    ...
    var imgNames = ['../images/01.jpg', '../images/02.jpg', '../images/03.jpg', '../images/04.jpg'];
    var counter = 0;

    if (!localStorage.visitCount) {
        //Add each item from the json file to the DOM
        $.each(data.recipes, function (key, val) {
            var recipe = '<article class>';
    ...
    function add() {
        ...
        //Variables
        var imageCheck = /(.*?)\.(jpg|bmp|jpeg|png)$/;
        var inputs = document.getElementById('dialogBox');
        var title = inputs.querySelector('#title').value;
        var desc = inputs.querySelector('#desc').value;
        var image = inputs.querySelector('#image').value;

        var type = inputs.querySelector('#type');
        var typeText = type.options[type.selectedIndex].text;

        var rating = inputs.querySelector('#rating');
        var ratingText = rating.options[rating.selectedIndex].text;
    ...
    $('#recipes').on('click', '#edit', function () {
        ...
        $('#editRecipe').click(function () {
            ...
            var imageCheck = /(.*?)\.(jpg|bmp|jpeg|png)$/;
            var inputs = document.getElementById('dialogBox');
            var title2 = inputs.querySelector('#title').value;
            var desc2 = inputs.querySelector('#desc').value;
            var image2 = inputs.querySelector('#image').value;

            var type2 = inputs.querySelector('#type');
            var typeText2 = type.options[type.selectedIndex].text;

            var rating2 = inputs.querySelector('#rating');
            var ratingText2 = rating.options[rating.selectedIndex].text;
